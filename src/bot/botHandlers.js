...
import { updateSeller } from '../repositories/sellerRepo.js';
...
export function registerBotHandlers(bot, sellerId) {
  ...
  bot.action(/^pay_accept:(.+)$/, async (ctx) => {
    ...
    if (telegramUserId) {
      // Unban if they were previously kicked (so they can rejoin after renewing)
      if (seller.private_group_chat_id && seller.telegram_bot_token) {
        try {
          await fetch(`https://api.telegram.org/bot${seller.telegram_bot_token}/unbanChatMember?chat_id=${seller.private_group_chat_id}&user_id=${telegramUserId}`);
        } catch (_) {}
      }
  ...
  bot.on('chat_join_request', async (ctx) => {
    ...
    // Store private_group_chat_id for cron to kick expired users later
    const seller = await getSellerById(sellerId);
    if (seller && !seller.private_group_chat_id) {
      await updateSeller(sellerId, { private_group_chat_id: groupChatId });
    }
